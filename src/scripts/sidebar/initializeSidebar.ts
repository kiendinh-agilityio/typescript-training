 ...
 */
const generateMainSidebar = (): string =>
...
const generateSubSidebar = (): string =>
  SUB_SIDEBAR_ITEMS.map((item: SidebarItem) => {
...
export const initializeSidebar = (): void => {
  ...
  const mainSidebarList = document.querySelector(
    '.main-sidebar-list',
  ) as HTMLElement;
  const subSidebarList = document.querySelector(
    '.sub-sidebar-list',
  ) as HTMLElement;
